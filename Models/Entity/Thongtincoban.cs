using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace QLDHS.Models.Entity
{
    public class Thongtincoban
    {
        public string MaLHS { get; set; }
        public string HoTen { get; set; }
        public DateTime? NgaySinh { get; set; }
        public string GioiTinh { get; set; }
        public string DanToc { get; set; }
        public string TonGiao { get; set; }
        public string Mien { get; set; }
        public string ThongTinLienLac { get; set; }
        public int? MaDoiTuong { get; set; }
        public string QueQuan { get; set; }
        public string NoiOHienNay { get; set; }
        public int? Khoa { get; set; }
        public int? BoMon { get; set; }
        public string SoHieuSiQuan { get; set; }
        public DateTime? NgayNhapNgu { get; set; }
        public DateTime? NgayVaoDoan { get; set; }
        public DateTime? Ngayvaodang { get; set; }
        public DateTime? Ngaychuyendangchinhthuc { get; set; }
        public string Conemcanbo { get; set; }
        public string Thanhphangiadinh { get; set; }
        public string Diachilienlacgiadinh { get; set; }
        public string Thongtinbo { get; set; }
        public string Thongtinme { get; set; }
        public string Donvitruockhidihoc { get; set; }
        public string Doantruong { get; set; }
        public string Linhvucthemanh { get; set; }



    }
}